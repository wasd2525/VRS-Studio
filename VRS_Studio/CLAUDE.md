...
Endpoints: `GET /` (page), `GET /status.json` (~2 Hz session snapshot), `GET /live.json` (~10 Hz live eye feed), `POST /api/start`, `POST /api/config?…`, `POST /api/end?reason=…`. **Parameters ride in the query string, not a request body** — that keeps the socket server from having to parse headers, content-length and chunked encoding. Because of that the request line is the size limit for a whole config: it allows `RequestLineLimit` (8 KB), and a line that overruns it is **dropped entirely** rather than handed on as a fragment, which would apply half a config silently.
...
**Ending a session** (`POST /api/end` → `PupilDataRecorder.EndSession`) abandons the run in progress and returns to the participant prompt, for a spoiled run or a handover to the next person. It writes a `SessionEnded` event (reason + tallies) **before** closing the CSV — otherwise a truncated file is indistinguishable from a crash downstream — falling back to `WriteEventMarkerRow()` if eye tracking is down and the normal sample path would not fire. Then it stops every paradigm coroutine, hides the stimuli, resets per-run state and restarts `RunSession()`.

Two things about it are load-bearing. It calls `StopAllCoroutines`, so it is **main-thread-only and must never be called from a coroutine** — the operator command queue drains from `Update()`, which is the intended caller. And `ResetSessionState()` deliberately **keeps** the stimulus configuration (luminance, sizes, durations, positions, requested eye mode): handing the headset over normally means the same protocol on a new person. Only `activeEye` resets, being a per-participant detection result rather than a setting. Gaze calibration also resets, since it is per-seating. On the page the button **arms before it fires** (click → "Confirm end" → click, 4 s timeout), because the page is usually driven from a phone and one stray tap would bin a run.

Note `EndRun()` — reaching the end of the trial sequence normally — still calls `Application.Quit()`, so the End button cannot cover that case; the app has to be relaunched for the next participant after a completed run.
